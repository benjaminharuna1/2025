...
const BulkAddResultsPage: React.FC = () => {
  ...
  const handleSubmitAll = async () => {
    ...
    const resultsToSubmit = marks
      .filter(mark => mark.marks !== '') // Only submit entries with marks
      .map(mark => ({
    ...
    try {
      await api.post('/results/bulk', { results: resultsToSubmit });
      alert('Results submitted successfully!');
      // Optionally, clear the form or redirect
      setMarks(students.map(student => ({ studentId: student._id, marks: '' })));
    } catch (err: any) {
                        ...
                        <tr key={student._id}>
                          ...
                          <td>
                            <IonInput
                              ...
                              onIonChange={(e) => handleMarkChange(student._id, e.detail.value!)}
